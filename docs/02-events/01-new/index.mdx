...
<Steps>
1. Um einen neuen Termin zu erfassen, muss man angemeldet sein.
    :::cards
    ![Anmelden](./images/login.png)
    ::br
    ![Erfolgreich angemeldet](./images/login-success.png)
    :::

2. Eigene Termine findet man unter dem Menüpunkt __Persönlich__, dort können auch neue Termine erstellt werden.
...
3. Die Eingabemaske öffnet sich und der Termin kann erfasst werden. Je nach Bildschirmgrösse wird die Eingabemaske direkt in der Zeile oder im Fenster angezeigt.
...
4. **Termindetails erfassen**. Eine Beschreibung aller Felder ist [👉 weiter unten](#eingabemaske) zu finden. Änderungen können über die Schaltfläche __:mdi[content-save]{.green} Speichern__ gespeichert werden.  
    Ein Termin muss also nicht auf anhieb vollständig erfasst werden. Er kann gespeichert und zu einem späteren Zeitpunkt weiter bearbeitet werden.
    ...
    :::
5. **Termin zur Überprufung einreichen.**  
    Ein neuer Termin kann während einem offenen Eingabefenster zur Überprüfung eingereicht werden. Eine Administrator\:in prüft und veröffentlicht den Termin. Erst nach der Veröffentlichung wird der Termin im Kalender angezeigt.
    ...
    <Tabs groupId="edit-mode">
        ...
        <TabItem value="window-mask" label="Fenstermodus">            
                ![--width=400px](./images/change-state-window-mode.png)
        </TabItem>
    </Tabs>
6. **Termin kontrollieren**
    Nach dem Einreichen soll die finale Version nochmals auf Richtigkeit kontrolliert werden.

    ![](./images/check-draft.png)
...
**Bestehende Termine** können auch ausserhalb der Eingabefenster aktualisiert werden, wobei Änderungen auch wieder von der Administration geprüft und veröffentlicht werden.
