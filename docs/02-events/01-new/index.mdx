...
5. ### Zur Überprüfung einreichen
    Ein neuer Termin kann während einem [👉 offenen zeitlichen Eingabefenster](#eingabefenster) zur Überprüfung eingereicht werden. Eine Administrator\:in prüft und veröffentlicht den Termin. Erst nach der Veröffentlichung wird der Termin im Kalender angezeigt.
...
<Dl>
    ...
    <dd>Das Publikum (Klassenstufe, Klasse oder Fachschaft) soll im Titel enthalten sein.</dd>
    <dt>Beschreibung</dt>
    ...
    <dd>
        ...
        - Lehrpersonen können direkt über das Suchfeld hinzugefügt werden. [👉 Ganze Fachschaften hinzufügen](#fachschaften-hinzufügen)
        ...
        :::
        :::danger[Eine Lehrperson wird nicht angezeigt?]
        Aktuell sind nur Lehrpersonen sichtbar, welche sich bereits im System angemeldet haben.
        :::
    </dd>
    <dt>Betrifft</dt>
    <dd style={{marginBottom: '1.2em 0'}}>
        <Badge 
            text="Schaltfläche anwählen für Beispiele [KLP], [SuS], [Alle]" 
            icon={mdiArrowDownThick} 
            size={0.7}
            color="orange" 
            iconSide="left" 
            className={styles.interactionHint}
            title="Klicke auf die verschiedenen Optionen um die betroffenen Gruppen anzuzeigen." 
        />
    ...
    <dd>
        <Badge 
            text="Schaltfläche anwählen für Beispiele [Teilweise], [Nein]" 
            size={0.7}
            icon={mdiArrowDownThick} 
            color="orange" 
            iconSide="left" 
            className={styles.interactionHint}
            title="Klicke auf die verschiedenen Optionen um die Beispiele anzuzeigen."
        />
...
:::info[Festlegung der zeitlichen Eingabefenster]
Die zeitlichen Eingabefenster werden von der Administration festgelegt und wie bisher in einer Orientierung kommuniziert.
Ein zeitliches Eingabefenster ist vom Start- bis zum Enddatum offen und ermöglicht in dieser Zeit, **neue Termine** einzureichen.
:::
...
## Fachschaften Hinzufügen

Es lassen sich auch ganze Fachschaften als Publikum hinzufügen. Die Fachschaftszugehörigkeit wird dabei auf Basis der unterrichteten Fächer der Lehrpersonen ermittelt.

<VideoGrid
    videos={[
        {
            src: require('./images/audience-fs.mp4').default,
            title: 'Alle Lehrpersonen einer Fachschaft hinzufügen',
            autoplay: true,
            maxHeight: '300px',
            loop: true
        }
